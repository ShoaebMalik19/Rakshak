...
const dangerZones = [
  { id: '1', name: 'Silk Board Junction', desc: 'High harassment reported', severity: 'high', reports: 14, latitude: 12.9165, longitude: 77.6229, radius: 500 },
  { id: '2', name: 'BTM Layout', desc: 'Poor lighting zone', severity: 'medium', reports: 7, latitude: 12.9165, longitude: 77.6101, radius: 400 },
  { id: '3', name: 'Majestic Bus Stand', desc: 'Theft reported frequently', severity: 'high', reports: 23, latitude: 12.9767, longitude: 77.5713, radius: 600 },
  { id: '4', name: 'Yeshwanthpur', desc: 'Suspicious persons reported', severity: 'medium', reports: 5, latitude: 13.0228, longitude: 77.5507, radius: 350 },
  { id: '5', name: 'KR Market', desc: 'Poorly lit, avoid after 9pm', severity: 'low', reports: 3, latitude: 12.9629, longitude: 77.5759, radius: 300 },
];

const safeZones = [
  { id: 's1', name: 'Indiranagar', desc: 'Well-lit, CCTV covered', latitude: 12.9784, longitude: 77.6408, radius: 500 },
  { id: 's2', name: 'Koramangala', desc: 'High footfall, safe area', latitude: 12.9352, longitude: 77.6245, radius: 400 },
  { id: 's3', name: 'Whitefield', desc: 'Security patrolled, 24/7', latitude: 12.9698, longitude: 77.7499, radius: 450 },
];

const route1 = [
  { latitude: 12.9716, longitude: 77.5946 },
  { latitude: 12.9750, longitude: 77.6100 },
  { latitude: 12.9784, longitude: 77.6250 },
  { latitude: 12.9800, longitude: 77.6400 },
];
const route2 = [
  { latitude: 12.9716, longitude: 77.5946 },
  { latitude: 12.9680, longitude: 77.6080 },
  { latitude: 12.9650, longitude: 77.6200 },
  { latitude: 12.9800, longitude: 77.6400 },
];
const route3 = [
  { latitude: 12.9716, longitude: 77.5946 },
  { latitude: 12.9600, longitude: 77.6000 },
  { latitude: 12.9580, longitude: 77.6200 },
  { latitude: 12.9800, longitude: 77.6400 },
];
...
export default function MapScreen() {
  ...
  const severityColor = (s: string) => s === 'high' ? '#C0445A' : s === 'medium' ? '#B07D3A' : '#4A90D9';
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {activeTab === 'route' && (
        <View style={{ flex: 1 }}>
          ...
          >
            ...
            <Marker coordinate={{ latitude: 12.9800, longitude: 77.6400 }} title="MG Road Metro" pinColor="#C0445A" />
          </MapView>
            ...
            {routesData.map(route => (
              <TouchableOpacity
                ...
                style={[styles.routeCard, {
                  ...
                  borderColor: selectedRoute === route.id ? route.color : 'transparent',
                  borderWidth: 2,
                }]}
              ...
              >
                ...
                {route.id === '1' && (
                  <View style={[styles.safestBadge, { backgroundColor: '#3A7D4420' }]}>
                    <Feather name="award" size={12} color="#3A7D44" />
                    <Text style={[styles.safestText, { color: '#3A7D44' }]}>Safest Route</Text>
                  </View>
                )}
                <View style={styles.routeMeta}>
                  {[
                    { icon: 'map-pin' as const, val: route.distance },
                    { icon: 'clock' as const, val: route.time },
                    { icon: 'sun' as const, val: route.lighting },
                    { icon: 'users' as const, val: route.crowd },
                  ].map((m, i) => (
      ...
      {activeTab === 'zones' && (
        <View style={{ flex: 1 }}>
            ...
            {dangerZones.map(zone => (
              <React.Fragment key={zone.id}>
                <Circle
                  ...
                  radius={zone.radius}
                  fillColor={severityColor(zone.severity) + '30'}
                ...
                <Marker
                  ...
                  pinColor={severityColor(zone.severity)}
                />
              </React.Fragment>
            ))}
            {heatmapOn && safeZones.map(zone => (
              <React.Fragment key={zone.id}>
                <Circle
                  center={{ latitude: zone.latitude, longitude: zone.longitude }}
                  radius={zone.radius}
          ...
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: bottomPad + 90 }}>
            <Text style={[styles.zoneHeader, { color: colors.primary }]}>⚠ Danger Zones</Text>
            {dangerZones.map(zone => (
            ...
            {heatmapOn && (
              <>
                <Text style={[styles.zoneHeader, { color: '#3A7D44', marginTop: 8 }]}>✓ Safe Zones</Text>
                {safeZones.map(zone => (
                  <View key={zone.id} style={[styles.zoneCard, { backgroundColor: colors.card }]}>
                    <View style={[styles.zoneLeft, { backgroundColor: '#3A7D4420' }]}>
                      <Feather name="shield" size={18} color="#3A7D44" />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.zoneName, { color: colors.foreground }]}>{zone.name}</Text>
                      <Text style={[styles.zoneDesc, { color: colors.mutedForeground }]}>{zone.desc}</Text>
                    </View>
                    <View style={[styles.severityBadge, { backgroundColor: '#3A7D4420' }]}>
                      <Text style={[styles.severityText, { color: '#3A7D44' }]}>SAFE</Text>
                    </View>
                  </View>
                ))}
              </>
