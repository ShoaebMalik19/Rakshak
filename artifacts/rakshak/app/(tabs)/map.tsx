...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Modal, Animated, Platform, Alert } from 'react-native';
...
export default function MapScreen() {
  ...
  const openReport = () => { setReportVisible(true); Animated.spring(slideAnim, { toValue: 0, useNativeDriver: true, tension: 80, friction: 12 }).start(); };
  const closeReport = () => { Animated.timing(slideAnim, { toValue: 500, duration: 250, useNativeDriver: true }).start(() => setReportVisible(false)); };
  const submitReport = () => {
    closeReport();
    ...
    Alert.alert(
      ...
      [
        { text: "I'm Aware", style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            addActivity({ id: Date.now().toString(), text: `Entered danger zone: ${zone.name}`, time: 'Just now', icon: 'alert-circle' });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { paddingTop: topPad + 8, backgroundColor: colors.background }]}>
        ...
        <TouchableOpacity style={[styles.layerBtn, { backgroundColor: showAllZones ? colors.primary : colors.card }]} onPress={() => setShowAllZones(v => !v)}>
          <Ionicons name="layers" size={16} color={showAllZones ? '#fff' : colors.foreground} />
          <Text style={{ color: showAllZones ? '#fff' : colors.foreground, fontSize: 12, fontWeight: '600' }}>{showAllZones ? 'All Zones' : 'Danger Only'}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={[styles.tabs, { backgroundColor: colors.card, marginHorizontal: 16 }]}>
        {(['route', 'zones'] as const).map(tab => (
          <TouchableOpacity key={tab} style={[styles.tab, activeTab === tab && { backgroundColor: colors.primary }]} onPress={() => setActiveTab(tab)}>
            <Text style={[styles.tabText, { color: activeTab === tab ? '#fff' : colors.mutedForeground }]}>{tab === 'route' ? 'Safe Routes' : 'Danger Zones'}</Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        <View style={{ flex: 1 }}>
          <View style={{ position: 'relative' }}>
            <MapView style={styles.map} provider={PROVIDER_DEFAULT} initialRegion={BENGALURU} showsUserLocation={false}>
              {routesData.map(route => (
                <React.Fragment key={route.id}>
                  <Polyline coordinates={route.coords} strokeColor={route.color} strokeWidth={selectedRoute === route.id ? 5 : 2} lineDashPattern={selectedRoute === route.id ? undefined : [8, 4]} />
                  <Marker coordinate={route.coords[0]} title={route.name} pinColor={route.color}>
                    <View style={[styles.scoreMarker, { backgroundColor: route.color }]}><Text style={styles.scoreMarkerText}>{route.score}</Text></View>
                  </Marker>
                </React.Fragment>
              ))}
            ...
            <View style={styles.legend}>
              {[['#3A7D44', 'Safe'], ['#B07D3A', 'Caution'], ['#C0445A', 'Danger']].map(([color, label]) => (
                <View key={label} style={styles.legendRow}><View style={[styles.legendDot, { backgroundColor: color }]} /><Text style={styles.legendText}>{label}</Text></View>
              ))}
            </View>
          </View>
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: bottomPad + 90 }}>
            <View style={[styles.inputCard, { backgroundColor: colors.card, borderLeftColor: colors.primary }]}>
              <View style={styles.inputRow}><View style={[styles.dot, { backgroundColor: '#3A7D44' }]} /><TextInput style={[styles.routeInput, { color: colors.foreground }]} value={fromInput} onChangeText={setFromInput} placeholder="From" placeholderTextColor={colors.mutedForeground} /></View>
              <View style={[styles.divider, { backgroundColor: colors.border }]} />
              <View style={styles.inputRow}><View style={[styles.dot, { backgroundColor: colors.primary }]} /><TextInput style={[styles.routeInput, { color: colors.foreground }]} value={toInput} onChangeText={setToInput} placeholder="To" placeholderTextColor={colors.mutedForeground} /></View>
            </View>
            {routesData.map(route => (
              <TouchableOpacity key={route.id} style={[styles.routeCard, { backgroundColor: colors.card, borderLeftColor: route.color, borderColor: selectedRoute === route.id ? route.color : 'transparent', borderWidth: 2 }]} onPress={() => setSelectedRoute(route.id)} activeOpacity={0.8}>
                <View style={styles.routeTop}>
                  ...
                  <View style={[styles.scoreBadge, { backgroundColor: route.color }]}><Text style={styles.scoreText}>{route.score}/100</Text></View>
                </View>
                {route.id === '1' && <View style={styles.safestBadge}><Ionicons name="trophy" size={12} color="#3A7D44" /><Text style={styles.safestText}>Safest Route</Text></View>}
                <View style={styles.routeMeta}>
                  {[{ icon: 'location', val: route.distance }, { icon: 'time', val: route.time }, { icon: 'sunny', val: route.lighting }, { icon: 'people', val: route.crowd }].map((m, i) => (
                    <View key={i} style={styles.metaItem}><Ionicons name={m.icon as any} size={12} color={colors.mutedForeground} /><Text style={[styles.metaText, { color: colors.mutedForeground }]}>{m.val}</Text></View>
                  ))}
                </View>
              </TouchableOpacity>
            ))}
      ...
      {activeTab === 'zones' && (
        <View style={{ flex: 1 }}>
            ...
            <MapView style={styles.map} provider={PROVIDER_DEFAULT} initialRegion={BENGALURU}>
              {visibleZones.map(zone => (
                <React.Fragment key={zone.id}>
                  <Circle center={{ latitude: zone.latitude, longitude: zone.longitude }} radius={zone.radius} fillColor={severityFill(zone.severity)} strokeColor={severityColor(zone.severity)} strokeWidth={2} />
          ...
          <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: bottomPad + 90 }}>
            {/* Simulate zone entry */}
            <TouchableOpacity style={[styles.simulateBtn, { backgroundColor: '#C0445A15', borderColor: '#C0445A' }]} onPress={simulateRedZoneEntry}>
          ...
          <Animated.View style={[styles.bottomSheet, { backgroundColor: colors.card, transform: [{ translateY: slideAnim }] }]}>
            ...
            <View style={styles.incidentTypes}>
              {incidentTypes.map(type => (
                <TouchableOpacity key={type} style={[styles.typeChip, { backgroundColor: incidentType === type ? colors.primary : colors.secondary, borderColor: incidentType === type ? colors.primary : colors.border }]} onPress={() => setIncidentType(type)}>
...
const styles = StyleSheet.create({
  ...
  layerBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 8, paddingHorizontal: 14, borderRadius: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 2 },
  tabs: { flexDirection: 'row', borderRadius: 14, padding: 4, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 },
  tab: { flex: 1, paddingVertical: 10, borderRadius: 11, alignItems: 'center' },
  tabText: { fontSize: 14, fontWeight: '600' },
  map: { height: 220 },
  legend: { position: 'absolute', left: 12, bottom: 12, backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 10, padding: 8, gap: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 6, elevation: 3 },
  legendRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  legendDot: { width: 10, height: 10, borderRadius: 5 },
  legendText: { fontSize: 11, fontWeight: '600', color: '#333' },
  inputCard: { borderRadius: 16, padding: 4, marginBottom: 12, borderLeftWidth: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  routeCard: { borderRadius: 18, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  simulateBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 14, borderRadius: 14, borderWidth: 1.5, marginBottom: 16 },
  simulateBtnText: { fontSize: 14, fontWeight: '700' },
  zoneHeader: { fontSize: 16, fontWeight: '700', marginBottom: 10, marginTop: 4 },
  zoneCard: { flexDirection: 'row', alignItems: 'center', gap: 14, padding: 14, borderRadius: 16, marginBottom: 10, borderLeftWidth: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  ...
  fab: { position: 'absolute', right: 20, flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 14, paddingHorizontal: 20, borderRadius: 28, shadowColor: '#C0445A', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 8 },
  ...
  submitBtn: { height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', shadowColor: '#C0445A', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 6 },
