...
export default function DetectScreen() {
  ...
  const startScan = () => {
    ...
    setTimeout(() => {
      ...
      addScan({ id: Date.now().toString(), location: 'Current Location, Bengaluru', result, timestamp: new Date().toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) });
      addActivity({ id: Date.now().toString(), text: result === 'alert' ? 'Camera scan — potential threat detected' : 'Camera scan — all clear', time: 'Just now', icon: 'camera' });
    }, 4000);
  };
  ...
  const triggerFakeCall = () => {
    if (selectedDelay.seconds === 0) {
      ...
      const t = setInterval(() => {
        setCountdown(c => {
          if (c <= 1) { clearInterval(t); setCallState('incoming'); return 0; }
          return c - 1;
        });
      }, 1000);
    }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      {callState === 'incoming' && (
        <IncomingCall
          ...
          callerBg={selectedCaller.bg}
          callerColor={selectedCaller.color}
      ...
      <SOSModal visible={sosVisible} onCancel={() => setSOSVisible(false)} onComplete={() => { setSOSVisible(false); triggerSOS(); }} />
            ...
            {activeTab === 'camera' && (
              <>
                <View style={[styles.instructCard, { backgroundColor: colors.accent + '15' }]}>
                  ...
                  <Text style={[styles.instructText, { color: colors.foreground }]}>Point your phone around the room slowly while scanning</Text>
                </View>
                ...
                <TouchableOpacity style={[styles.scanBtn, { backgroundColor: scanning ? colors.muted : colors.primary }]} onPress={startScan} disabled={scanning}>
                  <Ionicons name={scanning ? 'reload' : 'search'} size={18} color={scanning ? colors.mutedForeground : '#fff'} />
                  <Text style={[styles.scanBtnText, { color: scanning ? colors.mutedForeground : '#fff' }]}>{scanning ? 'Scanning...' : 'Start Scan'}</Text>
                </TouchableOpacity>
                {scans.length > 0 && (
                  <>
                    ...
                    {scans.slice(0, 3).map(scan => (
                      <View key={scan.id} style={[styles.scanRecord, { backgroundColor: colors.card }]}>
                        ...
                        <Text style={[styles.scanResult, { color: scan.result === 'safe' ? colors.safe : colors.primary }]}>{scan.result === 'safe' ? 'Clear' : 'Alert'}</Text>
                      </View>
                    ))}
                  </>
                )}
              ...
              <>
                <View style={[styles.distressCard, { backgroundColor: colors.card }]}>
                  <View style={styles.distressHeader}>
                    ...
                    <TouchableOpacity style={[styles.toggle, { backgroundColor: distressListenerOn ? colors.primary : colors.muted }]} onPress={() => setDistressListener(!distressListenerOn)}>
                  ...
                  {distressListenerOn && (
                    <View style={styles.soundwave}>
                      {[...Array(9)].map((_, i) => {
                        ...
                        return (
                          <Animated.View key={i} style={[styles.bar, { backgroundColor: colors.primary, height: h, opacity: soundwaveAnim.interpolate({ inputRange: [0, 1], outputRange: [0.4, 1] }), transform: [{ scaleY: soundwaveAnim.interpolate({ inputRange: [0, 1], outputRange: [0.5 + i * 0.05, 1] }) }] }]} />
                        );
                      })}
                    </View>
                  )}
                  <Text style={[styles.listenStatus, { color: distressListenerOn ? colors.primary : colors.mutedForeground }]}>{distressListenerOn ? 'Listening for distress signals...' : 'Listener is off'}</Text>
                </View>
                <View style={[styles.settingsCard, { backgroundColor: colors.card }]}>
                  ...
                  <View style={styles.sensitivityRow}>
                    {(['Low', 'Medium', 'High'] as const).map(s => (
                      <TouchableOpacity key={s} style={[styles.sensBtn, { backgroundColor: sensitivity === s ? colors.primary : colors.secondary }]} onPress={() => setSensitivity(s)}>
                        <Text style={[styles.sensText, { color: sensitivity === s ? '#fff' : colors.foreground }]}>{s}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                  <TouchableOpacity style={[styles.simulateBtn, { backgroundColor: colors.primary + '20', borderColor: colors.primary }]} onPress={() => setSOSVisible(true)}>
                ...
                <View style={[styles.infoCard, { backgroundColor: colors.card }]}>
                  ...
                  <Text style={[styles.infoText, { color: colors.mutedForeground }]}>Rakshak listens in the background for your set keyword. When detected, it automatically triggers an SOS countdown and alerts your trusted contacts.</Text>
                </View>
              </>
            )}

            {/* Fake Call — redesigned */}
            {activeTab === 'fakecall' && (
              <>
                ...
                <View style={styles.callerCards}>
                  {callers.map(c => (
                    <TouchableOpacity
                      key={c.id}
                      style={[styles.callerCard, { backgroundColor: colors.card, borderColor: selectedCaller.id === c.id ? '#C0445A' : colors.border, borderWidth: selectedCaller.id === c.id ? 2 : 1 }, selectedCaller.id === c.id && styles.callerCardSelected]}
                ...
                <View style={styles.delayRow}>
                  {delays.map(d => (
                    <TouchableOpacity
                      key={d.label}
                      style={[styles.delayPill, { backgroundColor: selectedDelay.label === d.label ? '#C0445A' : colors.card, borderColor: selectedDelay.label === d.label ? '#C0445A' : colors.border }]}
                      onPress={() => setSelectedDelay(d)}
                    >
                      <Text style={[styles.delayText, { color: selectedDelay.label === d.label ? '#fff' : colors.foreground }]}>{d.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
...
const styles = StyleSheet.create({
  ...
  tabRow: { flexDirection: 'row', borderRadius: 14, padding: 4, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 },
  ...
  scanBtn: { height: 52, borderRadius: 26, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 20, shadowColor: '#C0445A', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 10, elevation: 4 },
  scanBtnText: { fontSize: 16, fontWeight: '700' },
  histTitle: { fontSize: 16, fontWeight: '700', marginBottom: 10 },
  scanRecord: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14, marginBottom: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  ...
  distressCard: { borderRadius: 20, padding: 20, marginBottom: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  ...
  settingsCard: { borderRadius: 20, padding: 20, marginBottom: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  ...
  simulateBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 12, borderRadius: 24, borderWidth: 1.5 },
  simulateText: { fontSize: 14, fontWeight: '700' },
  infoCard: { borderRadius: 18, padding: 16, marginBottom: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 2 },
  infoText: { fontSize: 13, lineHeight: 20 },
  // Fake Call
  fakeCallHeader: { borderRadius: 18, padding: 18, marginBottom: 18, borderLeftWidth: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  ...
  callerCard: { flex: 1, borderRadius: 16, padding: 12, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 3 },
  ...
  triggerBtn: { height: 56, borderRadius: 28, backgroundColor: '#C0445A', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 14, shadowColor: '#C0445A', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 14, elevation: 6 },
