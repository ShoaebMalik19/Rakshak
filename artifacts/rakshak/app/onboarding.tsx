...
export default function OnboardingScreen() {
  ...
  const [step, setStep] = useState(-1); // -1 = splash/landing
  ...
  // Animate permission checkmarks with delay
  useEffect(() => {
    if (step === 2 && grantedPerms.length > 0) {
      const idx = grantedPerms[grantedPerms.length - 1];
      Animated.spring(checkAnims[idx], { toValue: 1, useNativeDriver: true, tension: 200, friction: 10 }).start();
    }
  }, [grantedPerms]);
  ...
  const finish = () => {
    ...
    if (contactName.trim() && contactPhone.trim()) {
      addContact({ id: Date.now().toString(), name: contactName.trim(), phone: contactPhone.trim(), isPrimary: false, shareLocation: false });
    }
  ...
  // ONBOARDING STEPS
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: '#FAF6EF' }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <View style={styles.blobTR} />
      <View style={styles.blobBL} />
      <ScrollView
          ...
          {step === 2 && (
            <View style={styles.permsSection}>
              {permissions.map((p, i) => {
                ...
                return (
                  <TouchableOpacity key={i} style={[styles.permRow, { borderColor: granted ? '#3A7D44' : '#E0D6CC', backgroundColor: granted ? '#3A7D4408' : '#fff' }]} onPress={() => grantPerm(i)} activeOpacity={0.85}>
                    ...
                    <Animated.View style={[styles.permCheck, { backgroundColor: granted ? '#3A7D44' : '#E0D6CC', transform: [{ scale: checkAnims[i] }] }]}>
...
const styles = StyleSheet.create({
  ...
  splashIconWrap: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center', elevation: 8, shadowColor: '#C0445A', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.2, shadowRadius: 20, marginBottom: 24 },
  ...
  getStartedBtn: { width: '100%', height: 56, backgroundColor: '#C0445A', borderRadius: 28, alignItems: 'center', justifyContent: 'center', shadowColor: '#C0445A', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 14, elevation: 6 },
  ...
  permRow: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 16, borderWidth: 1.5, gap: 14, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.04, shadowRadius: 6, elevation: 2 },
  ...
  nextBtn: { width: '100%', height: 56, borderRadius: 28, alignItems: 'center', justifyContent: 'center', marginTop: 28, shadowColor: '#C0445A', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.25, shadowRadius: 14, elevation: 6 },
