...
export default function JourneyScreen() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const now = new Date();
      const startMs = new Date(`1970-01-01T${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}:00`).getTime();
  ...
  const statusIcon = (s: string): 'checkmark-circle' | 'alert-circle' | 'time' => {
    if (s === 'safe') return 'checkmark-circle';
    if (s === 'alerted') return 'alert-circle';
    return 'time';
  };

  const safeJourneys = (journeys || []).filter(j => j.status !== 'active');
  const safeContacts = (contacts || []).filter(c => c.isPrimary).map(c => c.name);
      ...
      >
        ...
        {activeJourney ? (
          <View style={[styles.activeCard, { backgroundColor: colors.primary + '10', borderColor: colors.primary }]}>
            ...
            <Text style={[styles.activeDest, { color: colors.foreground }]}>{String(activeJourney.destination || '')}</Text>
            <Text style={[styles.activeEta, { color: colors.mutedForeground }]}>
              Expected arrival: {String(activeJourney.expectedArrival || '')}
            </Text>
            ...
            <Text style={[styles.alertingText, { color: colors.mutedForeground }]}>
              Alerting: {safeContacts.length > 0 ? safeContacts.join(', ') : 'Amma'}
            </Text>
            <TouchableOpacity style={[styles.safeBtn, { backgroundColor: colors.safe }]} onPress={handleSafeArrival}>
          ...
          <View style={[styles.formCard, { backgroundColor: colors.card }]}>
            ...
            />
            <TouchableOpacity
              style={[styles.startBtn, { backgroundColor: destination && arrivalTime ? colors.primary : colors.muted }]}
              onPress={handleStart}
              disabled={!destination || !arrivalTime}
            >
              <Ionicons name="navigate" size={18} color={destination && arrivalTime ? '#fff' : colors.mutedForeground} />
        ...
        {safeJourneys.map(j => (
          <View key={j.id} style={[styles.historyCard, { backgroundColor: colors.card }]}>
            <View style={[styles.historyIcon, { backgroundColor: statusColor(j.status) + '20' }]}>
              <Ionicons name={statusIcon(j.status)} size={18} color={statusColor(j.status)} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[styles.historyDest, { color: colors.foreground }]}>{String(j.destination || '')}</Text>
              <Text style={[styles.historyDate, { color: colors.mutedForeground }]}>{String(j.date || '')} · {String(j.expectedArrival || '')}</Text>
            </View>
        ...
        {safeJourneys.length === 0 && (
          <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>No completed journeys yet.</Text>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  activeCard: { borderRadius: 20, padding: 20, borderWidth: 2, marginBottom: 24 },
  ...
  safeBtn: {
    ...
    shadowColor: '#3A7D44', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 4,
  },
  ...
  alertOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', alignItems: 'center', justifyContent: 'center' },
  alertModal: {
    width: 300, padding: 32, borderRadius: 24, alignItems: 'center',
    shadowColor: '#000', shadowOffset: { width: 0, height: 16 }, shadowOpacity: 0.3, shadowRadius: 24, elevation: 20,
  },
  alertIcon: { width: 80, height: 80, borderRadius: 40, alignItems: 'center', justifyContent: 'center', marginBottom: 20 },
  ...
  alertClose: { paddingVertical: 14, paddingHorizontal: 32, borderRadius: 24 },
});
