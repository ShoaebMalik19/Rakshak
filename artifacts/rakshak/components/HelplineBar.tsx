...
import { View, Text, StyleSheet, TouchableOpacity, Modal, Animated, useRef } from 'react-native';
import { Feather } from '@expo/vector-icons';
import { useColors } from '@/hooks/useColors';

const helplines = [
  { label: 'Police', number: '100', icon: 'shield' as const },
  { label: "Women's", number: '1091', icon: 'heart' as const },
...
export default function HelplineBar() {
  ...
  const handlePress = (h: typeof helplines[0]) => {
    setSelected(h);
  };
      ...
      <Modal transparent visible={!!selected} animationType="fade" onRequestClose={() => setSelected(null)}>
        <TouchableOpacity style={styles.overlay} onPress={() => setSelected(null)} activeOpacity={1}>
          <View style={[styles.dialModal, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.dialTitle, { color: colors.foreground }]}>
              Calling {selected?.label} Helpline
            </Text>
